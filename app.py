...
# Placeholder content for MINDWORK_HOMEPAGE_HTML, needed for the home route.
...
def search():
    ...
    # Check if the Gemini client is ready AND the user query doesn't include a mock suffix
    # The mock suffixes are for easy local testing without API calls.
    should_call_gemini = (
    ...
    # Generate the large set of static/mock/web results
    static_results = generate_general_results(query)
    
    # Combine the results. Place the Gemini result (if successful) at the top.
    results = []
    if gemini_result:
    ...
    # Render the search results template with the combined data
